"""
Do not depends on LangChain's @tool or .bind_tools for this example. This is a raw 
implementation of an agent loop with function calling.
This example is meant to show how the underlying mechanism of tool calling works,
without any abstractions
for different model you need to find different tool schema in their website and 
implement it by yourself, for example, in this example, we use ollama, so we need to implement the tool calling schema that ollama support, if you change to other model provider, you also need to change the tool calling schema to fit that model provider.
"""
